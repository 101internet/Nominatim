...
COPY ./ ./src
RUN cd ./src && git submodule update --recursive --init && \
...
# Apache configure
COPY docker/local.php /app/src/build/settings/local.php
COPY docker/nominatim.conf /etc/apache2/sites-enabled/000-default.conf
...
COPY docker/startapache.sh /app/startapache.sh
COPY docker/startpostgres.sh /app/startpostgres.sh
